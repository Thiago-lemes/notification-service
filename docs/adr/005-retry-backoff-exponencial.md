...
Canais de entrega externos (SMTP, WhatsApp, Webhooks) podem falhar temporariamente por indisponibilidade, timeout ou rate limiting. Sem uma estratégia de retry, notificações seriam perdidas em caso de falha transitória.
...
**Spring Retry:** considerado, mas não sobrevive a falhas de processo — se a aplicação cair durante o retry, a mensagem se perde. RabbitMQ garante durabilidade.